...
from backend.responses.types import LoggedResponse
from backend.router.types import *
import asyncio
import json
...
TRANSCRIPT_WINDOWS_TRUNCATE_LENGTH = 15
TRANSCRIPT_WINDOWS_SHORTEST_LENGTH = 5
...
RESPONSE_LOG_BASE_URL = "http://localhost:8000"
RESPONSE_LOG_ENDPOINT = "/api/v1/responses/"
...
log_query_client = AsyncClient(base_url=RESPONSE_LOG_BASE_URL)

# debug log
logger = logging.getLogger("uvicorn.error")

def citationsJSON(citations: list[Citation]):
    result = []
    for c in citations:
        result.append({"source_doc_id": c.source_doc_id,
...
async def router_suggest(meeting_id: str, no_record_mode: bool, top_k_context: Optional[int]):
    try:
        ...
        parsed_transcript = truncated_transcript.split()
        transcript_window = " ".join((parsed_transcript[-min(TRANSCRIPT_WINDOWS_TRUNCATE_LENGTH,len(parsed_transcript)):]))
        ...
        if len(truncated_transcript.split()) < TRANSCRIPT_WINDOWS_SHORTEST_LENGTH:
            ...
            await log_query_client.post(RESPONSE_LOG_ENDPOINT+"update",json={
                "request_id":request_id,"meeting_id":meeting_id,
        ...
        await log_query_client.post(RESPONSE_LOG_ENDPOINT+"update",json={
                ...
                "suggestion":{
                    ...
                    "rationale":llm_response.rationale,
                    "confidence":llm_response.confidence,
                    "citations":citationsJSON(llm_response.citations),
                    "latency_ms":latency_ms,
                },
                "transcript_window":transcript_window
            })
